...
def classify_anomalies(
    ...
    corroborated = z_keys & i_keys
    z_only       = z_keys - i_keys
    i_only       = i_keys - z_keys

    rows = []
    for key in corroborated:
        rows.append({time_col: key[0], geo_col: key[1],
                     "classification": "GENUINE_EVENT", "confidence": "HIGH"})
    for key in z_only:
        rows.append({time_col: key[0], geo_col: key[1],
                     "classification": "POSSIBLE_DATA_ERROR", "confidence": "MEDIUM"})
    for key in i_only:
        rows.append({time_col: key[0], geo_col: key[1],
                     "classification": "POSSIBLE_DATA_ERROR", "confidence": "LOW"})

    result = pd.DataFrame(rows).sort_values(["classification", time_col])
    logger.info(f"Classification: {len(corroborated)} genuine, "
...
if __name__ == "__main__":
    ...
    z_report   = z_detector.detect(df_total, country_col="geo",
                                   value_col="value", time_col="time")

    # 2. Isolation Forest
    df_agg = (
        df_total
        .groupby(["time", "geo"])
        .agg(total_applications=("value", "sum"),
    ...
    if not classification.empty:
        classification.to_csv(DATA_PROCESSED / "anomaly_classification.csv", index=False)
